...
class EP_Config {
	...
	public function get_ep_host( $force = false, $use_only_backups = false ) {
		...
		if ( true === $force ) {
			delete_transient( 'ep_last_good_host' );
		}

		$last_good_host = get_transient( 'ep_last_good_host' );
		...
		foreach ( $hosts as $host ) {
			...
			if ( true === ep_elasticsearch_alive( $host ) ) {
				...
				set_transient( 'ep_last_good_host', $host, apply_filters( 'ep_last_good_host_timeout', 3600 ) );
